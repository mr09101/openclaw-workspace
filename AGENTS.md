...
코딩·디버깅·리팩터링·코드리뷰에서는 `karpathy-guidelines` 스킬을 권위 원본으로 사용한다. `agent-workflow-guardrails`는 TeamDark/명확화/검증을 포함한 공통 운영 요약이며, 이 파일에서는 규칙을 반복하지 않는다.

최소 불변값:
- 중요한 가정과 해석 차이를 드러낸다.
- 요청에 직접 필요한 최소 범위만 바꾼다.
- 과설계와 무관한 정리를 피한다.
- 완료 전 관련 테스트·재현·실제 표면 검증 근거를 확인한다.

사소한 one-liner에는 과도한 절차를 강제하지 않는다.
